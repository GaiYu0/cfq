...
class LSTMModel(nn.Module):
    def __init__(self, ntok, ninp, nhid, nlayer):
        ...
        embeddings= read_embeddings('glove/glove.6B.50d.txt', ntok)

        # vocab_size = len(embeddings)
        # embedding_dim = len(embeddings[0])

        vocab_size, embedding_dim = embeddings.size()
        self.tok_encoder = nn.Embedding(vocab_size, embedding_dim)
        self.tok_encoder.load_state_dict({'weight': embeddings})

        #self.tok_encoder.weight = nn.Parameter(embeddings)
        ...
        self.lstm_encoder = nn.LSTM(embedding_dim, nhid // 2, nlayer, batch_first=True, bidirectional=True)
        self.pos_encoder = PositionalEncoding(embedding_dim, 0.0)
