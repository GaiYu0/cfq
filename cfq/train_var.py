...
from cfq.model_var import NounPhraseModel as Model
from cfq import DATA_DIR, RUN_DIR_ROOT
from cfq.data_var import CFQDataModule
...
flags.DEFINE_integer("num_epochs", 5, "Number of training epochs.", lower_bound=1)
...
flags.DEFINE_string("cfq_data_path", str(DATA_DIR / "cfq" / "var.npz"), "Main CFQ data path")
...
#   pl.seed_everything(FLAGS.seed)
    ...
    if FLAGS.mode == 'train':
        ...
        checkpoint_callback = ModelCheckpoint(monitor="valid/acc", save_top_k=5, save_last=True, mode="max")
