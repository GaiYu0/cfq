...
class RGCN(nn.Module):
    def __init__(self, num_toks, in_dim, h_dim, out_dim, num_rels, num_bases=-1, num_hidden_layers=1):
        super().__init__()
    ...
    def build_model(self):
        self.embedding = nn.Embedding(self.num_toks, self.in_dim)
    ...
    def build_input_layer(self):
        return RGCNLayer(self.in_dim, self.h_dim, self.num_rels, self.num_bases, activation=F.relu, is_input_layer=True)
    ...
    def forward(self, g):
        g.ndata["h"] = self.embedding(g.ndata["tok"])
