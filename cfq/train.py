...
flags.DEFINE_string("rel_vocab_path", "cfq/rel-vocab.pickle", "Rel. vocab path")
    ...
    def configure_optimizers(self):
        ...
        if FLAGS.warmup_epochs is None:
            ...
            scheduler = transformers.get_cosine_schedule_with_warmup(
                optimizer, FLAGS.warmup_epochs, FLAGS.num_epochs + 1, last_epoch=self.last_epoch
            )
...
def main(argv):
    ...
    rel_vocab = pickle.load(open(os.path.join(FLAGS.data_root_path, FLAGS.rel_vocab_path), "rb"))
    ...
    if FLAGS.resume_from_checkpoint is None:
        ...
        model = CFQTrainer.load_from_checkpoint(
            FLAGS.resume_from_checkpoint, tok_vocab=tok_vocab, rel_vocab=rel_vocab, last_epoch=ckpt["epoch"]
        )
